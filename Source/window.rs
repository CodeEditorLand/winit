...
impl WindowId {
	...
	/// behavior.**
	pub unsafe fn dummy() -> Self { WindowId(platform_impl::WindowId::dummy()) }
}
...
impl Window {
	...
	/// `WindowEvent::ScaleFactorChanged` events is the most robust way to
	/// track the DPI you need to use to draw.
	///
	/// ## Platform-specific
	///
	/// - **X11:** This respects Xft.dpi, and can be overridden using the
	///   `WINIT_X11_SCALE_FACTOR` environment variable.
	...
	#[inline]
	pub fn scale_factor(&self) -> f64 { self.window.scale_factor() }
...
impl Window {
	...
	///   _Note: this returns the same value as `outer_position`._
	...
	/// - **Web:** Returns the size of the canvas element. _Note: this returns
	///   the same value as `inner_size`._
	#[inline]
	pub fn outer_size(&self) -> PhysicalSize<u32> { self.window.outer_size() }
...
impl Window {
	...
	/// - **iOS:** Can only be called on the main thread.
	#[inline]
	pub fn set_visible(&self, visible:bool) { self.window.set_visible(visible) }

	/// Sets whether the window is resizable or not.
	///
	/// Note that making the window unresizable doesn't exempt you from handling
	/// `Resized`, as that event can still be triggered by DPI scaling,
	/// entering fullscreen mode, etc.
	///
	/// ## Platform-specific
	///
	...
	/// - **iOS:** Can only be called on the main thread.
	/// - **Wayland:** Does not support exclusive fullscreen mode and will no-op
	///   a request.
	...
	#[inline]
	pub fn fullscreen(&self) -> Option<Fullscreen> { self.window.fullscreen() }
	...
	/// Sets the window icon. On Windows and X11, this is typically the small
	/// icon in the top-left corner of the titlebar.
	///
	/// ## Platform-specific
	///
	...
	///
	/// X11 has no universal guidelines for icon sizes, so you're at the whims
	/// of the WM. That said, it's usually in the same ballpark as on Windows.
	#[inline]
...
impl Window {
	...
	/// - **Wayland:** The cursor is only hidden within the confines of the
	///   window.
	...
	/// - **Wayland:** Requires the cursor to be inside the window to be
	///   dragged.
...
/// ## Platform-specific
///
/// - **X11:** Sets the WM's `XUrgencyHint`. No distinction between `Critical`
///   and `Informational`.
