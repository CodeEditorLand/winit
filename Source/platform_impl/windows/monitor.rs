...
// Send is not implemented for HMONITOR, we have to wrap it and implement it
...
impl MonitorHandle {
	pub(crate) fn new(hmonitor:HMONITOR) -> Self { MonitorHandle(hmonitor) }
	...
	#[inline]
	pub fn native_identifier(&self) -> String { self.name().unwrap() }

	#[inline]
	pub fn hmonitor(&self) -> HMONITOR { self.0 }
