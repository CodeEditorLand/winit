	...
	pub fn new_dpi_unaware_any_thread() -> EventLoop<T> {
		...
		EventLoop {
			...
			window_target:RootELW {
				p:EventLoopWindowTarget { thread_id, thread_msg_target, runner_shared },
				_marker:PhantomData,
			},
		}
	}
...
fn main_thread_id() -> DWORD {
	...
	static INIT_MAIN_THREAD_ID:unsafe fn() = {
		unsafe fn initer() { MAIN_THREAD_ID = processthreadsapi::GetCurrentThreadId(); }
		initer
	};
...
fn dur2timeout(dur:Duration) -> DWORD {
	...
	dur.as_secs()
		...
		.map(
			|ms| {
				if ms > DWORD::max_value() as u64 { winbase::INFINITE } else { ms as DWORD }
			},
		)
		...
		match msg {
			...
			winuser::WM_PAINT => {
				if subclass_input.event_loop_runner.should_buffer() {
					...
					subclass_input
						.send_event(Event::RedrawRequested(RootWindowId(WindowId(window))));
					if managing_redraw {
					...
					for pointer_info in pointer_infos.iter().rev() {
						...
						subclass_input.send_event(Event::WindowEvent {
							...
							event:WindowEvent::Touch(Touch {
								...
								{
									TouchPhase::Ended
								} else if pointer_info.pointerFlags & winuser::POINTER_FLAG_UPDATE
									!= 0
								{
									TouchPhase::Moved
								} else {
							...
							(cursor_pos.x
								- (suggested_cursor_horizontal_ratio
									* (conservative_rect.right - conservative_rect.left) as f64)
									as LONG) - conservative_rect.left
						};
...
unsafe extern "system" fn thread_event_target_callback<T:'static>(
	...
	if msg != winuser::WM_PAINT {
		winuser::RedrawWindow(window, ptr::null(), ptr::null_mut(), winuser::RDW_INTERNALPAINT);
	}
			...
			winuser::WM_INPUT => {
				...
				if let Some(data) = raw_input::get_raw_input_data(lparam as _) {
					...
					if data.header.dwType == winuser::RIM_TYPEMOUSE {
						...
						let button_state =
							raw_input::get_raw_mouse_button_state(mouse.usButtonFlags);
	...
	let result = subclass_input.event_loop_runner.catch_unwind(callback).unwrap_or(-1);
	if subclass_removed {
