...
type SkipPointerFrameMessages =
	unsafe extern "system" fn(pointerId:UINT) -> BOOL;
...
type GetPointerTouchInfo = unsafe extern "system" fn(
	pointerId:UINT,
	touchInfo:*mut winuser::POINTER_TOUCH_INFO,
) -> BOOL;

type GetPointerPenInfo = unsafe extern "system" fn(
	pointId:UINT,
	penInfo:*mut winuser::POINTER_PEN_INFO,
) -> BOOL;
		...
		if thread_id != main_thread_id() {
			panic!(concat!(
				"Initializing the event loop outside of the main thread is a \
				 ...
				 really, absolutely need to create an EventLoop on a \
				 different thread, please use the `EventLoopExtWindows::",
				$fn_name,
				"` function."
			));
		}
...
impl<T:'static> EventLoop<T> {
	...
	pub fn new_dpi_unaware_any_thread() -> EventLoop<T> {
		...
		thread::spawn(move || {
			wait_thread(thread_id, send_thread_msg_target as HWND)
		});
...
fn get_wait_thread_id() -> DWORD {
	unsafe {
		...
		assert_eq!(
			...
			"this shouldn't be possible. please open an issue with Winit. \
			 error code: {}",
			result
		);
		msg.lParam as DWORD
...
fn wait_thread(parent_thread_id:DWORD, msg_window_id:HWND) {
	unsafe {
		...
		'main: loop {
			// Zeroing out the message ensures that the `WaitUntilInstantBox`
			// doesn't get double-freed if `MsgWaitForMultipleObjectsEx`
			// returns early and there aren't additional messages to process.
			msg = mem::zeroed();
			...
			if msg.message == *WAIT_UNTIL_MSG_ID {
				wait_until_opt =
					Some(*WaitUntilInstantBox::from_raw(msg.lParam as *mut _));
			...
			if let Some(wait_until) = wait_until_opt {
				let now = Instant::now();
				if now < wait_until {
					// MsgWaitForMultipleObjects tends to overshoot just a
					// little bit. We subtract 1 millisecond from the
					// requested time and spinlock for the remainder to
					// compensate for that.
...
///
/// Winit's API guarantees that all redraw events will be clustered together and
/// dispatched all at once, but the standard Windows message loop doesn't always
/// exhibit that behavior. If multiple windows have had redraws scheduled, but
/// an input event is pushed to the message queue between the `WM_PAINT` call
/// for the first window and the `WM_PAINT` call for the second window, Windows
/// will dispatch the input event immediately instead of flushing all the redraw
/// events. This function explicitly pulls all of Winit's redraw events out of
/// the event queue so that they always all get processed in one fell swoop.
///
...
unsafe fn public_window_callback_inner<T:'static>(
	...
	// rather than passing the closure to catch_unwind directly so that the
	// match body indendation wouldn't change and the git blame and history
	// would be preserved.
	let callback = || {
		match msg {
			...
			winuser::WM_EXITSIZEMOVE => {
				subclass_input.window_state.lock().set_window_flags_in_place(
					|f| f.remove(WindowFlags::MARKER_IN_SIZE_MOVE),
				);
				0
			},
			...
			winuser::WM_WINDOWPOSCHANGING => {
				...
				if let Some(ref mut fullscreen) = window_state.fullscreen {
					...
					let new_monitor = winuser::MonitorFromRect(
						&new_rect,
						...
						Fullscreen::Borderless(ref mut fullscreen_monitor) => {
							if new_monitor != ptr::null_mut()
								&& fullscreen_monitor
									.as_ref()
									.map(|monitor| {
										new_monitor != monitor.inner.hmonitor()
									})
									.unwrap_or(true)
							{
			...
			// WM_MOVE supplies client area positions, so we send Moved here
			// instead.
			winuser::WM_WINDOWPOSCHANGED => {
				use crate::event::WindowEvent::Moved;

				let windowpos = lparam as *const winuser::WINDOWPOS;
				if (*windowpos).flags & winuser::SWP_NOMOVE
					!= winuser::SWP_NOMOVE
				{
			...
			winuser::WM_CHAR | winuser::WM_SYSCHAR => {
				...
				if is_high_surrogate {
					subclass_input.window_state.lock().high_surrogate =
						Some(wparam as u16);
				} else if is_low_surrogate {
					let high_surrogate = subclass_input
						.window_state
						.lock()
						.high_surrogate
						.take();
					...
					if let Some(high_surrogate) = high_surrogate {
						...
						if let Some(Ok(chr)) =
							char::decode_utf16(pair.iter().copied()).next()
						{
							subclass_input.send_event(Event::WindowEvent {
								window_id:RootWindowId(WindowId(window)),
			...
			winuser::WM_MOUSEMOVE => {
				...
				if mouse_was_outside_window {
					...
					// Calling TrackMouseEvent in order to receive mouse leave
					// events.
					winuser::TrackMouseEvent(&mut winuser::TRACKMOUSEEVENT {
						cbSize:mem::size_of::<winuser::TRACKMOUSEEVENT>()
							as DWORD,
			...
			winuser::WM_KEYDOWN | winuser::WM_SYSKEYDOWN => {
				...
				if msg == winuser::WM_SYSKEYDOWN
					&& wparam as i32 == winuser::VK_F4
				{
					commctrl::DefSubclassProc(window, msg, wparam, lparam)
				} else {
					if let Some((scancode, vkey)) =
						process_key_params(wparam, lparam)
					{
						...
						// Windows doesn't emit a delete character by default,
						// but in order to make it consistent with the
						// other platforms we'll emit a delete character here.
			...
			winuser::WM_CAPTURECHANGED => {
				// lparam here is a handle to the window which is gaining mouse
				// capture. If it is the same as our window, then we're
				// essentially retaining the capture. This can happen if
				// `SetCapture` is called on our window when it already has the
				// mouse capture.
			...
			winuser::WM_TOUCH => {
				...
				) > 0
				{
					for input in &inputs {
						...
						if winuser::ScreenToClient(
							window,
							&mut location as *mut _,
						) == 0
						{
							continue;
						}
						...
						subclass_input.send_event(Event::WindowEvent {
							...
							event:WindowEvent::Touch(Touch {
								phase:if input.dwFlags
									& winuser::TOUCHEVENTF_DOWN != 0
								{
									TouchPhase::Started
								} else if input.dwFlags
									& winuser::TOUCHEVENTF_UP != 0
								{
									TouchPhase::Ended
								} else if input.dwFlags
									& winuser::TOUCHEVENTF_MOVE != 0
								{
									TouchPhase::Moved
								} else {
									continue;
								...
								location,
								force:None, /* WM_TOUCH doesn't support
			...
			| winuser::WM_POINTERUP => {
				if let (
					...
					*GET_POINTER_DEVICE_RECTS,
					...
					// The information retrieved appears in reverse
					// chronological order, with the most recent entry in the
					...
					for pointer_info in pointer_infos.iter().rev() {
						...
						// For the most precise himetric to pixel conversion we
						...
						subclass_input.send_event(Event::WindowEvent {
							...
							event:WindowEvent::Touch(Touch {
								phase:if pointer_info.pointerFlags
									& winuser::POINTER_FLAG_DOWN
									!= 0
								{
									TouchPhase::Started
								} else if pointer_info.pointerFlags
									& winuser::POINTER_FLAG_UP != 0
								{
									TouchPhase::Ended
								} else if pointer_info.pointerFlags
									& winuser::POINTER_FLAG_UPDATE
									!= 0
								{
									TouchPhase::Moved
								} else {
			...
			winuser::WM_SETFOCUS => {
				...
				for windows_keycode in event::get_pressed_keys() {
					let scancode = winuser::MapVirtualKeyA(
						windows_keycode as _,
						winuser::MAPVK_VK_TO_VSC,
					);
					let virtual_keycode =
			...
			winuser::WM_SETCURSOR => {
				let set_cursor_to = {
					let window_state = subclass_input.window_state.lock();
					// The return value for the preceding `WM_NCHITTEST` message
					// is conveniently provided through the low-order word
					// of lParam. We use that here since `WM_MOUSEMOVE`
					// seems to come after `WM_SETCURSOR` for a given cursor
					// movement.
					let in_client_area =
						LOWORD(lparam as DWORD) == winuser::HTCLIENT as WORD;
					if in_client_area {
					...
					|| window_state.max_size.is_some()
				{
					if let Some(min_size) = window_state.min_size {
						...
						let (width, height):(u32, u32) =
							util::adjust_size(window, min_size).into();
						(*mmi).ptMinTrackSize =
					...
					if let Some(max_size) = window_state.max_size {
						...
						let (width, height):(u32, u32) =
							util::adjust_size(window, max_size).into();
						(*mmi).ptMaxTrackSize =
							POINT { x:width as i32, y:height as i32 };
					}
				}
			...
			// Only sent on Windows 8.1 or newer. On Windows 7 and older user
			// has to log out to change DPI, therefore all applications are
			// closed while DPI is changing.
			winuser::WM_DPICHANGED => {
				use crate::event::WindowEvent::ScaleFactorChanged;

				// This message actually provides two DPI values - x and y.
				// However MSDN says that "you only need to use either the
				// X-axis or the Y-axis value when scaling your application
				// since they are the same". https://msdn.microsoft.com/en-us/library/windows/desktop/dn312083(v=vs.85).aspx
				...
				let style =
					winuser::GetWindowLongW(window, winuser::GWL_STYLE) as _;
				let style_ex =
					winuser::GetWindowLongW(window, winuser::GWL_EXSTYLE) as _;

				// New size as suggested by Windows.
				let suggested_rect = *(lparam as *const RECT);

				// The window rect provided is the window's outer size, not it's
				// inner size. However, win32 doesn't provide an
				// `UnadjustWindowRectEx` function to get the client rect from
				// the outer rect, so we instead adjust the window rect to get
				// the decoration margins and remove them from the outer
				// size.
				let margin_left:i32;
				let margin_top:i32;
				...
				{
					let adjusted_rect = util::adjust_window_rect_with_styles(
						window,
						style,
						style_ex,
						suggested_rect,
					...
					// margin_right = adjusted_rect.right -
					// suggested_rect.right; margin_bottom =
					// adjusted_rect.bottom - suggested_rect.bottom;
				}
				...
				// the restored size after exiting fullscreen (the restored
				// size is already DPI adjusted).
				let mut new_physical_inner_size = match allow_resize {
					// We calculate our own size because the default suggested
					// rect doesn't do a great job of preserving the
				...
				let new_outer_rect:RECT;
				{
					...
					let mut conservative_rect = RECT {
						...
						right:suggested_ul.0
							+ new_physical_inner_size.width as LONG,
						bottom:suggested_ul.1
							+ new_physical_inner_size.height as LONG,
					};
					...
					// If we're dragging the window, offset the window so that
					// the cursor's relative horizontal position in the
					// title bar is preserved.
					if dragging_window {
						let bias = {
							let cursor_pos = {
							...
							(cursor_pos.x
								- (suggested_cursor_horizontal_ratio
									* (conservative_rect.right
										...
										as f64) as LONG) - conservative_rect.left
						};
					...
					let conservative_rect_monitor =
						winuser::MonitorFromRect(&conservative_rect, 0);
					new_outer_rect = if conservative_rect_monitor
						== new_dpi_monitor
					{
						conservative_rect
					} else {
						let get_monitor_rect = |monitor| {
							let mut monitor_info = winuser::MONITORINFO {
								cbSize:mem::size_of::<winuser::MONITORINFO>()
									as _,
								..mem::zeroed()
							};
						...
						// factor. We calculate this by seeing which monitor
						...
						for _ in 0..abort_after_iterations {
							...
							if winuser::MonitorFromRect(&conservative_rect, 0)
								== new_dpi_monitor
							{
								break;
							}
			...
			winuser::WM_SETTINGCHANGE => {
				...
				let preferred_theme =
					subclass_input.window_state.lock().preferred_theme;
	...
	subclass_input.event_loop_runner.catch_unwind(callback).unwrap_or(-1)
}
...
unsafe extern "system" fn thread_event_target_callback<T:'static>(
	...
	let callback = || {
		match msg {
			...
			// modal loops to detect when the event queue has been emptied.
			// See `process_event` for more details.
			winuser::WM_PAINT => {
				winuser::ValidateRect(window, ptr::null());
				// If the WM_PAINT handler in `public_window_callback` has
				// already flushed the redraw events, `handling_events` will
				// return false and we won't emit a second
				...
				// Default WM_PAINT behaviour. This makes sure modals and popups
				// are shown immediatly when opening them.
				commctrl::DefSubclassProc(window, msg, wparam, lparam)
			},
				...
				if let Some(data) = raw_input::get_raw_input_data(lparam as _) {
					...
					if data.header.dwType == winuser::RIM_TYPEMOUSE {
						...
						if util::has_flag(
							...
							let delta = mouse.usButtonData as SHORT as f32
						...
						let button_state =
							raw_input::get_raw_mouse_button_state(
								mouse.usButtonFlags,
							);
						// Left, middle, and right, respectively.
						for (index, state) in button_state.iter().enumerate() {
							if let Some(state) = *state {
								// This gives us consistency with X11, since
								// there doesn't seem to be anything
								// else reasonable to do for a mouse
								// button ID.
						...
						if pressed || released {
							...
							let extended = util::has_flag(
								keyboard.Flags,
								winuser::RI_KEY_E0 as _,
							) | util::has_flag(
								keyboard.Flags,
								winuser::RI_KEY_E1 as _,
							);
			...
			_ if msg == *PROCESS_NEW_EVENTS_MSG_ID => {
				...
				// if the control_flow is WaitUntil, make sure the given moment
				// has actually passed before emitting NewEvents
					...
					subclass_input.event_loop_runner.control_flow()
				{
					...
					while Instant::now() < wait_until {
						if 0 != winuser::PeekMessageW(
							...
							// RDW_INTERNALPAINT set (i.e. doesn't
							// have an update region), PeekMessageW will remove
							// that window from the redraw queue even
							// though we told it not to remove messages from the
							// queue. We fix it by re-dispatching an internal
							// paint message to that window.
							if msg.message == winuser::WM_PAINT {
								let mut rect = mem::zeroed();
								if 0 == winuser::GetUpdateRect(
									msg.hwnd, &mut rect, 0,
								) {
									winuser::RedrawWindow(
										msg.hwnd,
