...
impl Window {
		...
		unsafe {
			...
			init(w_attr, pl_attr, event_loop).map(|win| {
				let file_drop_handler = if drag_and_drop {
					...
					// It is ok if the initialize result is `S_FALSE` because it
					// might happen that multiple windows are created on
					// the same thread.
					if ole_init_result == OLE_E_WRONGCOMPOBJ {
						panic!(
							"OleInitialize failed! Result was: \
							 `OLE_E_WRONGCOMPOBJ`"
						);
					} else if ole_init_result == RPC_E_CHANGED_MODE {
						panic!(
							"OleInitialize failed! Result was: \
							 `RPC_E_CHANGED_MODE`. Make sure other crates are \
							 not using multithreaded COM library on the same \
							 thread or disable drag and drop support."
						);
					}
					...
					let handler_interface_ptr = &mut (*file_drop_handler.data)
						.interface
						as LPDROPTARGET;
	...
	#[inline]
	pub fn hwnd(&self) -> HWND { self.window.0 }

	#[inline]
	pub fn hinstance(&self) -> HINSTANCE {
		unsafe {
			winuser::GetWindowLongPtrW(self.hwnd(), winuser::GWLP_HINSTANCE)
				as *mut _
		}
	}
	...
	pub fn set_cursor_grab(&self, grab:bool) -> Result<(), ExternalError> {
		...
		self.thread_executor.execute_in_thread(move || {
			let result = window_state
				.lock()
				.mouse
				.set_cursor_flags(window.0, |f| {
					f.set(CursorFlags::GRABBED, grab)
				})
	...
	#[inline]
	pub fn scale_factor(&self) -> f64 { self.window_state.lock().scale_factor }
	...
	pub fn drag_window(&self) -> Result<(), ExternalError> {
		unsafe {
			...
			let points = POINTS { x:points.x as SHORT, y:points.y as SHORT };
	...
	#[inline]
	pub fn id(&self) -> WindowId { WindowId(self.window.0) }
	...
	pub fn set_fullscreen(&self, fullscreen:Option<Fullscreen>) {
		...
		self.thread_executor.execute_in_thread(move || {
			...
			match (&old_fullscreen, &fullscreen) {
				...
				| (
					...
					let mut display_name =
						OsStr::new(&monitor.inner.native_identifier())
							.encode_wide()
							.collect::<Vec<_>>();
					...
					let mut native_video_mode =
						video_mode.video_mode.native_video_mode.clone();
			...
			unsafe {
				// There are some scenarios where calling
				// `ChangeDisplaySettingsExW` takes long enough to execute
				// that the DWM thinks our program has frozen and takes over
				// our program's window. When that happens, the `SetWindowPos`
				// call below gets eaten and the window doesn't get set to
				// the proper fullscreen position.
				//
				// Calling `PeekMessageW` here notifies Windows that our process
				// is still running fine, taking control back from the DWM
				// and ensuring that the `SetWindowPos` call below goes
				// through.
			...
			match &fullscreen {
				Some(fullscreen) => {
					...
					window_state.lock().saved_window =
	...
	#[inline]
	pub fn theme(&self) -> Theme { self.window_state.lock().current_theme }
}
...
impl Drop for Window {
	...
	fn drop(&mut self) {
		unsafe {
			// The window must be destroyed from the same thread that created
			// it, so we send a custom message to be handled by our callback
...
unsafe fn init<T:'static>(
	...
	window_flags
		.set(WindowFlags::NO_BACK_BUFFER, pl_attribs.no_redirection_bitmap);
	window_flags.set(WindowFlags::TRANSPARENT, attributes.transparent);
	// WindowFlags::VISIBLE and MAXIMIZED are set down below after the window
	// has been configured.
...
unsafe fn register_window_class(
	...
	let class = winuser::WNDCLASSEXW {
		...
		hIcon:h_icon,
		hCursor:ptr::null_mut(), /* must be null in order for cursor state to
		                          * work properly */
	...
	// trigger  an error, and because errors here are detected during
	// CreateWindowEx anyway. Also since there is no weird element in the
	// struct, there is no reason for this  call to fail.
...
impl Drop for ComInitialized {
	fn drop(&mut self) { unsafe { combaseapi::CoUninitialize() }; }
}
...
pub fn com_initialized() { COM_INITIALIZED.with(|_| {}); }
	...
	TASKBAR_LIST.with(|task_bar_list_ptr| {
		...
		if task_bar_list == ptr::null_mut() {
			...
			if hr != S_OK || (*task_bar_list).HrInit() != S_OK {
				// In some old windows, the taskbar object could not be created,
				// we just ignore it
				return;
			}
		...
		(*task_bar_list)
			.MarkFullscreenWindow(handle, if fullscreen { 1 } else { 0 });
	})
}

unsafe fn force_window_active(handle:HWND) {
	// In some situation, calling SetForegroundWindow could not bring up the
	// window, This is a little hack which can "steal" the foreground window
	// permission We only call this function in the window creation, so it
	// should be fine. See : https://stackoverflow.com/questions/10740346/setforegroundwindow-only-working-while-visual-studio-is-open
	let alt_sc = winuser::MapVirtualKeyW(
		winuser::VK_MENU as _,
