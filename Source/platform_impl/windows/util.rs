...
pub unsafe fn status_map<T, F:FnMut(&mut T) -> BOOL>(mut fun:F) -> Option<T> {
	...
	if fun(&mut data) != 0 { Some(data) } else { None }
}

fn win_to_err<F:FnOnce() -> BOOL>(f:F) -> Result<(), io::Error> {
	if f() != 0 { Ok(()) } else { Err(io::Error::last_os_error()) }
}
...
pub fn adjust_size(hwnd:HWND, size:PhysicalSize<u32>) -> PhysicalSize<u32> {
	...
	let rect =
		RECT { left:0, right:width as LONG, top:0, bottom:height as LONG };
...
pub(crate) fn set_inner_size_physical(window:HWND, x:u32, y:u32) {
	unsafe {
		let rect = adjust_window_rect(
			window,
			RECT { top:0, left:0, bottom:y as LONG, right:x as LONG },
		)
...
pub fn set_cursor_clip(rect:Option<RECT>) -> Result<(), io::Error> {
	unsafe {
		let rect_ptr =
			rect.as_ref().map(|r| r as *const RECT).unwrap_or(ptr::null());
...
impl CursorIcon {
	pub(crate) fn to_windows_cursor(self) -> *const wchar_t {
		match self {
			...
			| CursorIcon::Grabbing
			| CursorIcon::Move
			| CursorIcon::AllScroll => winuser::IDC_SIZEALL,
			CursorIcon::EResize
			| CursorIcon::WResize
			| CursorIcon::EwResize
...
pub(super) fn get_function_impl(
	...
	let function_ptr =
		unsafe { GetProcAddress(module, function.as_ptr() as LPCSTR) };
