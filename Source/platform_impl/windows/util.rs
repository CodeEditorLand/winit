...
pub fn adjust_size(hwnd:HWND, size:PhysicalSize<u32>) -> PhysicalSize<u32> {
	...
	let rect = RECT { left:0, right:width as LONG, top:0, bottom:height as LONG };
...
pub(crate) fn set_inner_size_physical(window:HWND, x:u32, y:u32) {
	unsafe {
		let rect =
			adjust_window_rect(window, RECT { top:0, left:0, bottom:y as LONG, right:x as LONG })
...
pub fn is_focused(window:HWND) -> bool { window == unsafe { winuser::GetActiveWindow() } }
