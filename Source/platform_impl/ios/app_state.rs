...
struct AppState {
	// This should never be `None`, except for briefly during a state
	// transition.
...
impl AppState {
	...
	unsafe fn get_mut() -> RefMut<'static, AppState> {
		// basically everything in UIKit requires the main thread, so it's
		// pointless to use the std::sync APIs.
	...
	fn wakeup_transition(&mut self) -> Option<EventWrapper> {
		// before `AppState::did_finish_launching` is called, pretend there is
	...
	fn events_cleared_transition(&mut self) {
		...
		match (old, new) {
			...
			(_, ControlFlow::Exit) => {
				...
				// it is not possible to quit an iOS app gracefully and
				// programatically
...
pub unsafe fn did_finish_launching() {
	...
	for window in windows {
		...
		if count > 1 {
			// Do a little screen dance here to account for windows being
			// created before `UIApplicationMain` is called. This fixes
			// visual issues such as being offcenter and sized incorrectly.
			// Additionally, to fix orientation issues, we gotta reset the
			// `rootViewController`.
			//
			// relevant iOS log:
			// ```
