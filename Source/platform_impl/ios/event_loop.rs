...
impl<T> EventLoopProxy<T> {
	fn new(sender:Sender<T>) -> EventLoopProxy<T> {
		unsafe {
			...
			extern fn event_loop_proxy_handler(_:*mut c_void) {}
...
fn setup_control_flow_observers() {
	unsafe {
		// begin is queued with the highest priority to ensure it is processed
		// before other observers
		...
		// Core Animation registers its `CFRunLoopObserver` that performs
		// drawing operations in `CA::Transaction::ensure_implicit` with a
		// priority of `0x1e8480`. We set the main_end priority to be 0, in
		// order to send MainEventsCleared before RedrawRequested. This value
		// was chosen conservatively to guard against apple using different
		// priorities for their redraw observers in different OS's or on
		// different devices. If it so happens that it's too conservative, the
		// main symptom would be non-redraw events coming in after
		// `MainEventsCleared`.
		//
		// The value of `0x1e8480` was determined by inspecting stack traces and
		// the associated registers for every `CFRunLoopAddObserver` call on
		// an iPad Air 2 running iOS 11.4.
		//
		// Also tested to be `0x1e8480` on iPhone 8, iOS 13 beta 4.
		extern fn control_flow_main_end_handler(
			_:CFRunLoopObserverRef,
			activity:CFRunLoopActivity,
			_:*mut c_void,
		) {
		...
		// end is queued with the lowest priority to ensure it is processed
		// after other observers
