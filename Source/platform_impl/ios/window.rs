...
impl Inner {
	...
	pub fn request_redraw(&self) {
		unsafe {
			if self.gl_or_metal_backed {
				// `setNeedsDisplay` does nothing on UIViews which are directly
				// backed by CAEAGLLayer or CAMetalLayer. Ordinarily the OS
				// sets up a bunch of UIKit state before calling drawRect: on a
				// UIView, but when using raw or gl/metal for drawing this
				// work is completely avoided.
				//
				// The docs for `setNeedsDisplay` don't mention `CAMetalLayer`;
				// however, this has been confirmed via testing.
				//
	...
	pub fn id(&self) -> WindowId { self.window.into() }
...
impl WindowId {
	pub unsafe fn dummy() -> Self { WindowId { window:std::ptr::null_mut() } }
}
...
impl From<&mut Object> for WindowId {
	fn from(window:&mut Object) -> WindowId { WindowId { window:window as _ } }
}

impl From<id> for WindowId {
	fn from(window:id) -> WindowId { WindowId { window } }
}
