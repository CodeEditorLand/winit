...
impl IdRef {
	pub fn new(inner:id) -> IdRef { IdRef(inner) }
	...
	pub fn non_nil(self) -> Option<IdRef> { if self.0 == nil { None } else { Some(self) } }
}
...
pub unsafe fn ns_string_id_ref(s:&str) -> IdRef { IdRef::new(NSString::alloc(nil).init_str(s)) }
...
#[allow(dead_code)]
pub unsafe fn open_emoji_picker() { let () = msg_send![NSApp(), orderFrontCharacterPalette: nil]; }

pub extern fn yes(_:&Object, _:Sel) -> BOOL { YES }
