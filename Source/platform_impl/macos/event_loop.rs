...
impl<T> EventLoop<T> {
	...
	pub fn window_target(&self) -> &RootWindowTarget<T> { &self.window_target }
	...
	where
		...
		// This transmute is always safe, in case it was reached through `run`,
		// since our lifetime will be already 'static. In other cases caller
		// should ensure that all data they passed to callback will actually
		// outlive it, some apps just can't move everything to event loop, so
...
pub fn stop_app_on_panic<F:FnOnce() -> R + UnwindSafe, R>(
	...
	match catch_unwind(f) {
		...
		Err(e) => {
			...
			// and we need to know in those callbacks if the application is
			// currently panicking
			{
...
impl<T> Clone for Proxy<T> {
	fn clone(&self) -> Self { Proxy::new(self.sender.clone()) }
}
