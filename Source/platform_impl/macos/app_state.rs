...
impl AppState {
	...
	pub fn launched(app_delegate:&Object) {
		...
		HANDLER.set_ready();
		HANDLER.waker().start();
		let create_default_menu =
			unsafe { get_aux_state_mut(app_delegate).create_default_menu };
	...
	pub fn cleared(panic_info:Weak<PanicInfo>) {
		...
		if HANDLER.should_exit() {
			unsafe {
				...
				let dialog_is_closing =
					HANDLER.dialog_is_closing.load(Ordering::SeqCst);
				let pool = NSAutoreleasePool::new(nil);
				if !INTERRUPT_EVENT_LOOP_EXIT.load(Ordering::SeqCst)
					&& !dialog_open && !dialog_is_closing
				{
					let () = msg_send![app, stop: nil];
					// To stop event loop immediately, we need to post some
					// event here.
...
unsafe fn window_activation_hack(ns_app:id) {
	...
	loop {
		...
		// `UnownedWindow::new` This way we preserve the user's desired
		// initial visiblity status TODO: Also filter on the type/"level" of
