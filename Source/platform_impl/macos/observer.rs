...
pub struct CFRunLoopObserverContext {
	pub version:CFIndex,
	pub info:*mut c_void,
	pub retain:Option<extern fn(info:*const c_void) -> *const c_void>,
	pub release:Option<extern fn(info:*const c_void)>,
	pub copyDescription:Option<extern fn(info:*const c_void) -> CFStringRef>,
}
...
pub struct CFRunLoopSourceContext {
	pub version:CFIndex,
	pub info:*mut c_void,
	pub retain:Option<extern fn(*const c_void) -> *const c_void>,
	pub release:Option<extern fn(*const c_void)>,
	pub copyDescription:Option<extern fn(*const c_void) -> CFStringRef>,
	pub equal:Option<extern fn(*const c_void, *const c_void) -> ffi::Boolean>,
	pub hash:Option<extern fn(*const c_void) -> CFHashCode>,
	pub schedule:Option<extern fn(*mut c_void, CFRunLoopRef, CFRunLoopMode)>,
	pub cancel:Option<extern fn(*mut c_void, CFRunLoopRef, CFRunLoopMode)>,
	pub perform:Option<extern fn(*mut c_void)>,
}
...
where
	...
	// Asserting unwind safety on this type should be fine because `PanicInfo`
	// is `RefUnwindSafe` and `Rc<T>` is `UnwindSafe` if `T` is
	// `RefUnwindSafe`.
...
impl RunLoop {
	unsafe fn get() -> Self { RunLoop(CFRunLoopGetMain()) }
...
impl Default for EventLoopWaker {
	fn default() -> EventLoopWaker {
		...
		unsafe {
			...
			// far into the future, but that gets changed to fire immediately
