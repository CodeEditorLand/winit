...
impl WindowId {
	pub unsafe fn dummy() -> Self { WindowId(0) }
}
...
impl Window {
			...
			window.connect_button_press_event(|window, event| {
				if event.get_button() == 1 {
					if let Some(gdk_window) = window.get_window() {
						...
						// recieve the click properly when resize isn't needed
	...
	pub fn current_monitor(&self) -> Option<RootMonitorHandle> { todo!() }

	#[inline]
	pub fn available_monitors(&self) -> VecDeque<MonitorHandle> { todo!() }
...
fn hit_test(window:&gdk::Window, cx:f64, cy:f64) -> WindowEdge {
	...
	let edge = match result {
		...
		// has to be bigger than 7. otherwise it will match the number with a
		// variant of WindowEdge enum and we don't want to do that also if the
		// number ever change, makke sure to change it in the
		// connect_button_press_event for window and webview
		_ => WindowEdge::__Unknown(8),
	};

	// FIXME: calling `window.begin_resize_drag` seems to revert the cursor back
	// to normal style
	window.set_cursor(
		Cursor::from_name(
			&display,
