...
/// windows. For example on Linux creating an event loop opens a connection to
/// the X or Wayland server.
///
...
pub struct EventLoop<T:'static> {
	...
	pub(crate) _marker:::std::marker::PhantomData<*mut ()>, /* Not Send nor
	                                                         * Sync */
}
...
/// closure within a given run loop. The only exception to this is `Exit` which,
/// once set, cannot be unset. Changes are **not** persistent between multiple
/// calls to `run_return` - issuing a new call will reset the control flow to
/// `Poll`.
///
...
pub enum ControlFlow {
	...
	///   `requestAnimationFrame` fires but sometimes the events in the queue
	///   may be sent before the next `requestAnimationFrame` callback, for
	///   example when the scaling of the page has changed. This should be
	///   treated as an implementation detail which should not be relied on.
	Poll,
	...
	/// *sticky* - once set, `control_flow` cannot be changed from `Exit`, and
	/// any future attempts to do so will result in the `control_flow`
	/// parameter being reset to `Exit`.
	Exit,
}

impl Default for ControlFlow {
	#[inline(always)]
	fn default() -> ControlFlow { ControlFlow::Poll }
}
...
impl EventLoop<()> {
	...
	/// platforms, but is imposed to eliminate any nasty surprises when porting
	/// to platforms that require it. `EventLoopExt::new_any_thread` functions
	/// are exposed in the relevant `platform` module if the target platform
	/// supports creating an event loop on any thread.
	///
	/// Usage will result in display backend initialisation, this can be
	/// controlled on linux using an environment variable `WINIT_UNIX_BACKEND`.
	/// Legal values are `x11` and `wayland`. If it is not set, winit will try
	/// to connect to a wayland connection, and if it fails will fallback on
	/// x11. If this variable is set with any other value, winit will panic.
	///
	/// ## Platform-specific
	///
	/// - **iOS:** Can only be called on the main thread.
	pub fn new() -> EventLoop<()> { EventLoop::<()>::with_user_event() }
}
...
impl<T> EventLoop<T> {
	...
	/// Hijacks the calling thread and initializes the winit event loop with the
	/// provided closure. Since the closure is `'static`, it must be a `move`
	/// closure if it needs to access any data from the calling context.
	///
